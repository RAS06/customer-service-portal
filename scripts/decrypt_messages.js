...
async function main() {
  ...
  // Expect columns containing ciphertext_hex, nonce_hex, tag_hex
  const idx = {
...
// Additional: decrypt sessions.csv and survey_responses.csv
...
function decryptSurveys(projectRoot, key) {
  ...
  for (let r = 1; r < rows.length; r++) {
    ...
    // decrypt messages snapshot if present
    ...
    if (messages_snapshot_text) {
      try {
        ...
        const decrypted = mobj.map(it => {
          // each item may have ciphertext, nonce, tag or ciphertext_hex naming
...
async function mainAll() {
  ...
  // decrypt messages (existing behavior)
  await main();
  // decrypt sessions
